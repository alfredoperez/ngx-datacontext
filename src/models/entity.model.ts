...
export class BaseEntity {
  ...
  public deserialize(input: any): BaseEntity {
    for (const property in input) {
      ...
      if (
        ...
        if (Array.isArray(result)) {
          (this as any)[property] = result.splice(0);
        } else {
          (this as any)[property] = Object.assign({}, result);
        }
      } else if (this.isDate(input[property])) {
        (this as any)[property] = new Date(input[property]);
      } else {
        (this as any)[property] = input[property];
      }
    }
  ...
  private deserializeEntity(entity: any, data: any) {
