...
const config = {
  libraryName: 'ngx-datacontext',
  unscopedLibraryName: 'ngx-datacontext',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build', ['clean'], (cb) => {
  runSequence('compile', 'test', 'npm-package', 'rollup-bundle', 'fix-es5', cb);
});
...
gulp.task('build-watch-no-tests', (cb) => {
  runSequence('compile', 'npm-package', 'rollup-bundle', 'fix-es5', cb);
});
...
gulp.task('remove-firebase-esm5', function () {
  ...
  gulp.src(path)
    .pipe(replace(/module\.exports =/g, '// REMOVED'))
...
gulp.task('fix-es5', () => {
  runSequence('remove-firebase-esm5',
    'remove-firebase-esm2015',
    'remove-firebase-esm5',
    'remove-firebase-esm2015'
  );
});
...
gulp.task('serve:demo', () => {
  return execDemoCmd('serve --preserve-symlinks --aot --proxy-config proxy.conf.json', {
    cwd: `${config.demoDir}`
  });
});

gulp.task('serve:demo-hmr', () => {
  return execDemoCmd('serve --hmr -e=hmr --preserve-symlinks --aot --proxy-config proxy.conf.json', {
    cwd: `${config.demoDir}`
  });
});
